...
% Loop over subjects and extract beta values from commercial folders using ROI masks.
% Beta values are stored in voxel x trial matrices, one per ROI.
% Each ROI's data is saved separately per subject, along with a trial-by-trial correlation matrix.
...
% Beta mapping info 
beta_info_file = '/Volumes/Chi/Model3_PRE/Com-Beta-Maps_PRE.txt';
    ...
    if exist(beta_file, 'file')
        ...
        if size(roi_combined_beta_values, 2) > 1
            ...
            correlation_filename = fullfile(base_spm_results_dir, ['Model3_sub-' subjID], ...
                ['Model3_sub-' subjID '_' roi_name '_correlation_matrix_PRE.mat']);
